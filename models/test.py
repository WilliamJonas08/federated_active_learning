...
#TODO : comprendre pourquoi 2 classes identiques sont nécessaires ?
...
def test_img_local(net_g, dataset, args,idx=None,indd=None, user_idx=-1, idxs=None):
    """
    Returns test accuracy of one user
...
def test_img_local_all(net, args, dataset_test, dict_users_test,w_locals=None,w_glob_keys=None, indd=None,dataset_train=None,dict_users_train=None, return_all=False):
        ...
        for idx in range(args.num_users):
            ...
            # Evaluation
            net_local.eval()
            # TODO : see why ponderation of loss and accuracy
            if 'femnist' in args.dataset or 'sent140' in args.dataset:
                acc, loss =  test_img_local(net_local, dataset_test, args,idx=dict_users_test[idx],indd=indd, user_idx=idx)
                tot += len(dataset_test[dict_users_test[idx]]['x'])

                acc_test_local[idx] = acc*len(dataset_test[dict_users_test[idx]]['x'])
                loss_test_local[idx] = loss*len(dataset_test[dict_users_test[idx]]['x'])

            else:
                acc, loss = test_img_local(net_local, dataset_test, args, user_idx=idx, idxs=dict_users_test[idx]) 
                tot += len(dict_users_test[idx])

                acc_test_local[idx] = acc*len(dict_users_test[idx])
                loss_test_local[idx] = loss*len(dict_users_test[idx])
